...
**After SHA**: `2607c5d61630beb9521972494f385f863a7b199c` (`2607c5d`)
...
### After 에서 새로 드러난 지적 — 상품 카드 `p21.jpg`

Hero 를 걷어내자 Lighthouse 가 다음 항목을 지적하기 시작했다 (5-5).
...
- **LCP 요소가 아니다.** 스크롤 아래 카드라 LCP·FCP 어느 쪽에도 들어가지 않는다.
  실제로 이 지적이 남은 채로 공식 조건(no throttling)에서 모바일·데스크탑 모두 **성능 100점**이다.
- **파일 자체가 무거운 원본이다.** 같은 `w=828` 를 거친 다른 13장은 3~19 kB 인데 이것만 72 kB 다.
  코드가 아니라 `public/images/products/p21.jpg` 원본의 문제이고,
  이는 `src/app/api/**` 와 함께 과제가 제공한 자산이다.
- **개입 수단이 품질 저하뿐이다.** `next/image` 는 이미 붙어 있어 남은 수단은 `quality` 를 낮추는 것인데,
  과제가 금지한 "품질을 낮춰 수치만 줄이는 것"에 정확히 해당한다.

> 이 항목은 **개선이 만든 부산물**이다. 7.4MB 짜리 항목이 사라지자 그 다음 크기가 임계선 위로 올라온 것이지,
> 없던 문제가 생긴 게 아니다. Before 리포트에도 같은 파일이 같은 크기로 들어 있었다.
...
#### 패널 filmstrip — Hero 지연 소멸 (1-3 대조)
...
| 항목                       | Before                       | After                         |
| -------------------------- | ---------------------------- | ----------------------------- |
| Hero 이미지가 채워진 시점  | 4번째 슬라이드 **24,151 ms** | **1번째 프레임**              |
| 그때까지 그 자리에 있던 것 | 베이지 빈 박스               | Hero 이미지 (이미 완성)       |
| 마지막에 채워지는 것       | Hero 이미지                  | Hero **문구** (스트리밍 대상) |

**무엇이 마지막에 오는지가 바뀌었다.** Before 는 데이터와 무관한 이미지가 24초를 기다렸고,
After 는 데이터에 묶인 문구만 기다린다. 변경 1(렌더링 경계를 데이터 소유권대로 나눈다)이
의도한 결과가 filmstrip 에 그대로 나타난다.

> 이 filmstrip 은 Lighthouse 리포트에 포함된 것이라 별도 Performance 녹화 없이 확인된다.
> Performance 패널 녹화는 아래 Network 대조와 함께 남긴다.
...
| transferred       | **8.0 MB**             |       |
| Finish            | 46.43 s                |       |
| DOMContentLoaded  | 1.29 s                 |       |
| Load              | **45.70 s**            |       |
| Hero 행 전송·Time | 7,546 kB · **45.12 s** |       |
...
| 로딩 · 에러 · 빈 상태 · 재시도        |                                                              |
| Hero 시각적 크기 · 비율 · 문구 · 품질 | ✅ LCP element 표시 크기 Before 와 동일 (332×415 · 1136×639) |
| FSD 의존 방향 · 슬라이스 Public API   | ✅ `eslint-plugin-boundaries` 위반 0                         |
| `pnpm check`                          | ✅ test 41 pass · lint · typecheck · build 전부 통과         |
