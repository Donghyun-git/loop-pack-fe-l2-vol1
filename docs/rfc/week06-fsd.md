...
| 목록 4xx                          | `/products?page=-1` 접속                               | 인라인 안내, 필터 폼 유지, 경계로 전파 안 됨    | ⬜ 브라우저 미관측 |
| 목록 5xx·네트워크                 | 서버를 내린 뒤 필터 조작 (NetworkError)                | 결과 영역만 fallback, 필터 폼 유지, 재시도 동작 | ⬜ 브라우저 미관측 |
| 목록 빈 결과                      | 결과가 없는 검색어 입력                                | "조건에 맞는 상품이 없습니다"                   | ⬜ 브라우저 미관측 |
| 홈 조회 실패                      | 서버를 내린 뒤 홈 새로고침                             | 홈 섹션 fallback + 재시도                       | ⬜ 브라우저 미관측 |
| 렌더 중 임시 `throw`              | `HomePage` 최상단에 `throw new Error('probe')` 후 제거 | `error.tsx` fallback, `reset` 복구, Header 생존 | ⬜ 브라우저 미관측 |

> **⬜ 항목은 코드 경로를 따라간 예상이지 관측 결과가 아니다.** API 계층(mock 응답 상태코드)과 빌드·타입·린트는 확인했고, 화면에 실제로 무엇이 그려지는지는 브라우저로 눌러봐야 확정된다. 확인 후 라벨만 바꾼다.
>
...
`page` 는 nuqs `parseAsInteger` 라 **음수를 정상 정수로 통과**시키고, `toSearchQueryParams` 가 그대로 실어 보낸다. mock 은 `isPositiveInteger` 검사에서 400 을 준다.
...
(`page=0` 은 `toSearchQueryParams` 의 `if (query.page)` 에서 falsy 라 전송되지 않는다.)
...
**막지 못하는 것**: Public API 우회(`@/entities/cart/model/useCartStore` 처럼 슬라이스 내부 경로 직접 참조)는 이 두 규칙으로 걸리지 않는다. `boundaries/entry-point` 규칙을 추가하면 막을 수 있는데, `index.ts`를 둘 슬라이스와 두지 않을 슬라이스가 섞여 있어(3단계 결정) 규칙이 복잡해진다. 이번에는 넣지 않고 사람이 리뷰에서 본다.
...
| ----------------- | --------------------------------- | ---- | -------------- |
| 수정한 슬라이스   | `features/product-filter` 두 파일 | ⬜   | ⬜             |
| 변경한 Public API | 없음                              | ⬜   | ⬜             |
| 새로 생긴 의존    | 없음                              | ⬜   | ⬜             |
